...
sub discover_userid {
    $cmd = "$runat $remote_pre env $remote_post";
    ($crc, $rc, $buf) = piperun($cmd, "grep EC2_USER_ID | awk -F'=' '{print \$2}'", "ubero");
    if ($rc || !$buf || $buf eq "") {
	doexit(1, "FAILED: ($crc, $rc, $buf) = $cmd\n");
    }
    $current_artifacts{"userid"} = "$buf";
    return(0);
}
...
sub authorize_ssh_from_usergroup {
    my $dgroup = shift @_ || $current_artifacts{group} || "default";
    my $suser = shift @_ || "admin";
    my $sgroup = shift @_ || "default";

    if (! $dgroup || $dgroup eq "") {
	doexit(1, "FAILED: invalid destination group '$dgroup'\n");
    }
    if ( (!$sgroup || $sgroup eq "") || (!$suser || $suser eq "") ) {
	doexit(1, "FAILED: invalid source user/group '$suser/$sgroup'\n");
    }
    
    sleep($library_sleep);
    
    $cmd = "$runat $remote_pre $prefix-authorize $dgroup -P tcp -p 22 -o $sgroup -u $suser $remote_post";
    ...
    $cleanup_artifacts{"rules"} .= "$dgroup -P tcp -p 22 -o $sgroup -u $suser,";
